...
from mavros_msgs.msg import Altitude, ExtendedState, HomePosition, State, \
                            WaypointList
from mavros_msgs.srv import CommandBool, CommandHome, ParamGet, SetMode, WaypointClear, \
                            WaypointPush
...
class MavrosCommon():
    ...
    def set_home(self, timeout):
        rospy.loginfo("setting home")
        loop_freq = 1
        rate = rospy.Rate(loop_freq)
        
        for i in xrange(timeout * loop_freq):
            #if self.home_position.geo == geo:
            #    rospy.loginfo("set home success")
            #    break
            #else:
            try:
                res = self.set_home_srv(True, 37, 120 ,5)
                if not res.success:
                    rospy.logerr("failed to set home command")
                else:
                    rospy.loginfo("set home success")
                    break
            except rospy.ServiceException as e:
